...
function App() {
  ...
  const [recipe, setRecipe] = useState(initialRecipe); // recipe : stateful object
  ...
  useEffect(() => {
    setPrepared(recipe.ingredients.every((i) => i.prepared)); // every() : return a Boolean value based on every item that matches the criteria
  }, [recipe]); // runs only when the recipe object changes
